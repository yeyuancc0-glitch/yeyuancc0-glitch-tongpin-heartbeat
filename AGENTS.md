...
- Vercel 项目：`yeyuancc0-glitchs-projects/tongpin-heartbeat` 仅作为旧 Vercel 生成域名的兜底跳转发布渠道；用户入口、邮件链接、PWA、推送授权和验收都直接使用 `https://tongpin.fancah.tech`。旧自定义域名 `https://app.fanch.tech` 已从 Vercel alias 移除并停止使用，不再作为发布、验证、跳转或对外入口。
