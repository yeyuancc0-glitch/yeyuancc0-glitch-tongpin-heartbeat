...
- guest / 暂不登录的首页界面要和绑定后保持同一套视觉与布局，只在点击留言、上传、写信、快捷互动等双人功能时提示先登录并绑定，不要给 guest 单独做一套空态首页。
...
- 全局临时提示（toast / snackbar / 站内轻提示）统一使用手机消息提醒式样：同一时间只显示一个，固定在当前视口顶部安全区下方，不能挂在整页文档顶部导致页面滚动到下方时看不到。
- 页面内的共用说明块（如 `InlineNotice`）应作为普通内联内容出现在对应卡片/表单流里；不要通过 portal 或 fixed/absolute 居中浮层展示，避免和全局 toast 重叠或长期遮挡设置页内容。
...
- Vercel 项目：`yeyuancc0-glitchs-projects/tongpin-heartbeat` 仅作为 Vercel 生成域名的兜底跳转发布渠道；用户入口、邮件链接、PWA、推送授权和验收都直接使用 `https://tongpin.fancah.tech`。旧自定义域名 `https://app.fanch.tech` 不保留，不再作为发布、验证、跳转或对外入口；Vercel 项目 domains 只应保留 `tongpin-heartbeat.vercel.app` 这类生成域名，并通过 `vercel.json` 永久跳转到新域名。
