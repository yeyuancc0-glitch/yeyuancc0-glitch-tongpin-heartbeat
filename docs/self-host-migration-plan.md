...
这个项目现在的后端不是一个独立 API，而是直接绑定在 Supabase 平台上。前端通过 `@supabase/supabase-js` 直连 Auth、Postgres、Storage、Realtime 和 RPC；`packages/db/migrations` 里承载 schema、RLS、RPC、trigger、cron；`supabase/functions` 里还有推送 worker 和云宠 AI。

如果要“完全脱离 Supabase”，就不能只改数据库地址，必须同时替换：
...
优化后的执行路线见 `docs/self-host-optimized-roadmap.md`。实际操作以该路线为主：先稳定 staging 基建，再做自建 API / BFF 兼容层，最后才讨论生产切换。
...
- 腾讯云公网入口当前会拦截未备案 staging 域名：HTTP 返回 DNSPod webblock，HTTPS SNI 表现为 TLS EOF；这需要 ICP 备案或改用 Cloudflare Tunnel / 境外入口解决
...
- `api-staging.fanch.tech`：测试 API
- `assets-staging.fanch.tech`：测试图片访问
- `minio-staging.fanch.tech`：测试对象存储控制台，仅临时开放或加 Basic Auth
- `api.fanch.tech`：生产 API
- `assets.fanch.tech`：生产图片访问
...
| Storage | `apps/app/lib/supabase/storage.ts` | MinIO / S3 + 服务器签名 URL |
...
3. 确认 staging 域名公网入口方案：ICP 备案直连或 Cloudflare Tunnel / 境外入口。
4. 实现真实自建 API，逐步替换 Supabase 直连能力。

验收：

- 服务器内部 `https://api-staging.fanch.tech/health` 返回正常；公网域名访问需等待 ICP 备案或 Tunnel / 境外入口方案
...
- `app.fanch.tech`：静态前端
- `api-staging.fanch.tech`：第一阶段 API
- `assets-staging.fanch.tech`：第一阶段对象访问
- `minio-staging.fanch.tech`：第一阶段 MinIO 控制台，仅维护期临时开放或加 Basic Auth
- `api.fanch.tech`：生产 API，切流前才启用
- `assets.fanch.tech`：生产对象访问或 CDN，切流前才启用
- `minio.fanch.tech`：生产维护入口，不长期公网开放
- `auth.fanch.tech`：第二阶段引入 Keycloak 时再启用
...
6. 已在 Cloudflare 添加 `api-staging.fanch.tech` 和 `assets-staging.fanch.tech` 指向 `81.71.9.118`。
7. 已验证 Caddy 为 `api-staging.fanch.tech` 签发证书，服务器内部 HTTPS `/health` 返回 HTTP/2 `200` 和 `status: ok`。
8. 腾讯云公网入口当前拦截未备案 staging 域名；下一步先决定 ICP 备案或 Cloudflare Tunnel / 境外入口，再验证容器重启、自恢复、日志和备份，之后开始写自建 API 和替换前端 Supabase 调用。

第一阶段完成前，生产前端仍继续使用 Supabase 和 Vercel，不影响当前线上访问。
